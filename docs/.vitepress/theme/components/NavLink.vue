<template v-if="!noIcon">
  <div id="article-container" class="flink">
    ...
    <div class="flink-desc">{{ desc }}</div>
    <div class="flink-list">
      <div v-for="(item) in items" s class="flink-list-item"><a :href="item.link" :title="item.title"
          rel="external nofollow" target="_blank"><img :data-lazy-src="item.icon" :src="item.icon" calt="chatGPT"
            class="flink-avatar entered loaded" data-ll-status="loaded"
            onerror="this.parentElement.style.display='none'">
          <div class="img-alt is-center">{{ item.title }}</div>
          <div class="flink-item-info"><span class="flink-item-name">{{ item.title }}</span><span :title="item.desc"
              class="flink-item-desc">{{ item.desc }}</span></div>
        </a></div>
    </div>
  </div>
</template>
